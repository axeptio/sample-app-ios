...
4. `package.json` is updated to the new version (`package-lock.json` is `.gitignore`d in this repo, so it is not tracked or committed by the release)
5. A GPG-signed git tag is created (e.g. `v2.4.0`)
6. A GitHub release is published
7. The release commit — `CHANGELOG.md`, `package.json` — is pushed back to `develop` (signed as `axeptio-bot`, tagged `[skip ci]` so it doesn't re-trigger the workflow)

> ⚠️ `@semantic-release/git` commits those two files **only**. It does not touch the iOS project files, so `MARKETING_VERSION`, `Info.plist` and the SDK pin from the list above still have to be updated in the PR that precedes the release. Run `npm run version:sync` after bumping `package.json` to propagate the first two.
